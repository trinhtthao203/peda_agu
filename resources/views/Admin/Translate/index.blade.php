...
@section('css')
<link href="{{ env('ASSETS_URL') }}assets/libs/select2/select2.min.css" rel="stylesheet" type="text/css" />
@endsection
@section('body')
<div class="row">
                ...
                @foreach($arr_lang as $lang_key => $lang_text)
                    @if($lang_key != app()->getLocale())
                        <a href="{{ env('APP_URL') }}{{ $lang_key }}/admin/translate">
                            <img src="{{ env('APP_ASSETS') }}assets/backend/images/flags/{{ $lang_key }}.jpg" alt="user-image" class="mr-1" height="18" style="border:1px solid #ccc;"> <span class="align-middle">{{ $arr_lang[$lang_key] }} </span>
                        </a>
                    @endif
                @endforeach
            </h3>
