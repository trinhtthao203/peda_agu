...
<div class="row">
    <div class="col-12">
        <div class="card-box">
            <h3 class="m-t-0"><a href="{{ env('APP_URL') }}{{ app()->getLocale() }}/admin/tuyen-sinh/thong-tin" class="btn btn-primary btn-sm"><i class="mdi mdi-reply-all"></i> {{ __('Trở về') }}</a> {{ __('Sửa Thông tin Tuyển sinh') }}</h3>
            <form action="{{ env('APP_URL') }}{{ app()->getLocale() }}/admin/tuyen-sinh/thong-tin/update" method="post" id="dinhkemform" enctype="multipart/form-data">
                {{ csrf_field() }}
                ...
                <div class="form-body">
                    ...
                    @php
                        if(old('ten') != null){
                            ...
                            $date_post = old('date_post'); $published = old('published');
                            ...
                            $published = isset($ds['$published']) ? $ds['$published'] : 0;
                        } else {
                            $ten = '';$noi_dung = '';$slug='';$thu_tu=0;$id_cat='';$mo_ta = ''; $date_post = App\Http\Controllers\ObjectController::setDate();$published = 0;
                        }
                    @endphp
                            ...
                            <select name="id_cat[]" id="id_cat" required class="form-control select2" multiple data-placeholder="{{ __('Chọn danh mục') }}">
                                ...
                                @if($arr_cats)
                                    @foreach($arr_cats as $key =>  $cat)
                                        <option value="{{ $key }}" @if(in_array($key,$id_cat)) selected @endif>{{ $cat }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-2 switchery-demo">
                            Published:&nbsp;&nbsp;&nbsp;
                            <input type="checkbox" name="published" id="published" class="js-switch" data-plugin="switchery" @if($published == 1) checked="checked" @endif data-color="#009efb" value="1"/>
                        </div>
                    </div>
