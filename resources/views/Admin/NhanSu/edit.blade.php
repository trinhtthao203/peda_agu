            ...
            <div class="card-body">
                <form action="{{ route('admin-nhan-su-update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <ul class="nav nav-tabs mb-4" id="langTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active font-weight-bold" id="vi-tab" data-toggle="tab"
                                data-bs-toggle="tab" href="#vi" role="tab">🇻🇳 Tiếng Việt</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link font-weight-bold" id="en-tab" data-toggle="tab" data-bs-toggle="tab"
                                href="#en" role="tab">🇬🇧 English</a>
                        </li>
                    </ul>
                        ...
                        <div class="tab-pane fade show active" id="vi" role="tabpanel">
                            <div class="row">
                                <div class="col-md-6 form-group mb-3">
                                    <label class="font-weight-bold text-gray-700">Họ và Tên <span
                                ...
                                <div class="col-md-6 form-group mb-3">
                                    ...
                                    <input type="text" name="hoc_ham_hoc_vi" class="form-control"
                                        value="{{ $ds->hoc_ham_hoc_vi }}">
                                </div>
                            </div>
                        ...
                        <div class="tab-pane fade" id="en" role="tabpanel">
                            <div class="row">
                                <div class="col-md-6 form-group mb-3">
                                    ...
                                    <input type="text" name="ho_ten_en" class="form-control"
                                        value="{{ $ds->ho_ten_en }}">
                                </div>
                                <div class="col-md-6 form-group mb-3">
                                    <label class="font-weight-bold text-gray-700">Academic Title / Degree (EN)</label>
                                    <input type="text" name="hoc_ham_hoc_vi_en" class="form-control"
                                        value="{{ $ds->hoc_ham_hoc_vi_en }}">
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12 form-group mb-3">
                                    ...
                                    <input type="text" name="chuyen_nganh_en" class="form-control"
                                        value="{{ $ds->chuyen_nganh_en }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        <label class="font-weight-bold text-gray-700">Đơn vị công tác <span
                                class="text-danger">*</span></label>
                        @php
                            $existingDepts = $ds->departments ?? [];
                            $deptCount = count($existingDepts) > 0 ? count($existingDepts) : 1;
                        @endphp
                        <div id="department-list" data-initial-count="{{ $deptCount }}">
                            @if (!empty($existingDepts))
                                @foreach ($existingDepts as $i => $entry)
                                    <div class="department-row border rounded p-3 mb-2 bg-light"
                                        data-index="{{ $i }}">
                                        <div class="row align-items-end">
                                            <div class="col-md-3 form-group mb-2">
                                                <label class="small text-muted">Đơn vị</label>
                                                <select name="departments[{{ $i }}][department_id]"
                                                    class="form-control" required>
                                                    <option value="">-- Chọn đơn vị --</option>
                                                    @foreach ($departments as $dept)
                                                        <option value="{{ $dept->_id }}"
                                                            {{ isset($entry['department_id']) && $entry['department_id'] == $dept->_id ? 'selected' : '' }}>
                                                            {{ $dept->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-3 form-group mb-2">
                                                <label class="small text-muted">Chức vụ (VI) <span
                                                        class="text-danger">*</span></label>
                                                <input type="text" name="departments[{{ $i }}][chuc_vu]"
                                                    class="form-control" required value="{{ $entry['chuc_vu'] ?? '' }}">
                                            </div>
                                            <div class="col-md-3 form-group mb-2">
                                                <label class="small text-muted">Chức vụ (EN)</label>
                                                <input type="text" name="departments[{{ $i }}][chuc_vu_en]"
                                                    class="form-control" value="{{ $entry['chuc_vu_en'] ?? '' }}">
                                            </div>
                                            <div class="col-md-1 form-group mb-2">
                                                <label class="small text-muted">Thứ tự</label>
                                                <input type="number" name="departments[{{ $i }}][thu_tu]"
                                                    class="form-control" value="{{ $entry['thu_tu'] ?? 0 }}"
                                                    min="0" max="9999">
                                            </div>
                                            <div class="col-md-1 form-group mb-2 d-flex align-items-center">
                                                <div class="form-check mt-3">
                                                    <input type="checkbox"
                                                        name="departments[{{ $i }}][is_primary]"
                                                        class="form-check-input" value="1"
                                                        id="is_primary_{{ $i }}"
                                                        {{ !empty($entry['is_primary']) ? 'checked' : '' }}>
                                                    <label class="form-check-label small"
                                                        for="is_primary_{{ $i }}">Chính</label>
                                                </div>
                                            </div>
                                            <div class="col-md-1 form-group mb-2 d-flex align-items-end">
                                                <button type="button"
                                                    class="btn btn-sm btn-outline-danger btn-remove-dept"
                                                    {{ count($existingDepts) <= 1 ? 'disabled' : '' }}>✕</button>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <div class="department-row border rounded p-3 mb-2 bg-light" data-index="0">
                                    <div class="row align-items-end">
                                        ...
                                        <div class="col-md-3 form-group mb-2">
                                            <label class="small text-muted">Chức vụ (VI) <span
                                                    class="text-danger">*</span></label>
                                            <input type="text" name="departments[0][chuc_vu]" class="form-control"
                                                required>
                                        </div>
                                        <div class="col-md-3 form-group mb-2">
                                            <label class="small text-muted">Chức vụ (EN)</label>
                                        ...
                                        <div class="col-md-1 form-group mb-2">
                                            <label class="small text-muted">Thứ tự</label>
                                            <input type="number" name="departments[0][thu_tu]" class="form-control"
                                                value="0" min="0" max="9999">
                                        </div>
                                        <div class="col-md-1 form-group mb-2 d-flex align-items-center">
                                            <div class="form-check mt-3">
                                                <input type="checkbox" name="departments[0][is_primary]"
                                                    class="form-check-input" value="1" id="is_primary_0" checked>
                        ...
                        <button type="button" id="btn-add-dept" class="btn btn-sm btn-outline-success mt-1">➕ Thêm đơn
                            vị</button>
                    </div>

                    <div class="row">
                        <div class="col-md-6 form-group mb-4">
                            <label class="font-weight-bold text-gray-700">Thay đổi ảnh chân dung (Để trống nếu giữ
                                nguyên)</label>
                            <input type="file" name="hinh_anh"
                                class="form-control-file border p-2 w-100 rounded bg-light mb-2" accept="image/*">
                            @if ($ds->hinh_anh)
                                <div class="text-xs text-muted">File hiện tại: <span
                                        class="text-monospace">{{ $ds->hinh_anh }}</span></div>
                            @endif
                        </div>
                        <div class="col-md-6 form-group mb-4">
                            <label class="font-weight-bold text-gray-700">Thay đổi tệp Lý lịch khoa học PDF</label>
                            <input type="file" name="ly_lich_khoa_hoc"
                                class="form-control-file border p-2 w-100 rounded bg-light mb-2" accept=".pdf">
                            @if (isset($ds->ly_lich_khoa_hoc['aliasname']))
                                <div class="text-xs text-muted">File hiện tại: <span
                                        class="text-monospace text-success">{{ $ds->ly_lich_khoa_hoc['aliasname'] }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
...
@section('js')
    <script>
        (function() {
            var deptListEl = document.getElementById('department-list');
            var rowCount = parseInt(deptListEl.getAttribute('data-initial-count') || '1', 10);

            var deptOptions = `<option value="">-- Chọn đơn vị --</option>` +
                @foreach ($departments as $dept)
                    `<option value="{{ $dept->_id }}">{{ addslashes($dept->name) }}</option>` +
                @endforeach
            ``;
            ...
            deptListEl.addEventListener('click', function(e) {
