...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('admin-nhan-su-add', [app()->getLocale()]) }}" class="btn btn-primary shadow-sm">➕ {{ __('Thêm giảng viên mới') }}</a>
    </div>
            ...
            <form method="GET" action="{{ route('admin-nhan-su', [app()->getLocale()]) }}">
                <div class="form-row align-items-end">
                    <div class="col-md-5 mb-2">
                        <label class="small font-weight-bold text-gray-700 mb-1">{{ __('Họ tên hoặc Email') }}</label>
                        <input type="text" name="keyword" class="form-control" placeholder="{{ __('Nhập họ tên (VI/EN) hoặc email...') }}" value="{{ request('keyword') }}">
                    </div>
                    <div class="col-md-4 mb-2">
                        ...
                        <select name="department_id" class="form-control">
                            ...
                            <option value="{{ $dept->_id }}" {{ request('department_id') == (string)$dept->_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-3 mb-2 d-flex">
                        ...
                        <a href="{{ route('admin-nhan-su', [app()->getLocale()]) }}" class="btn btn-outline-secondary">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0 text-sm">
                    ...
                    <tbody class="text-gray-600 divide-y">
                        ...
                        $displayDegree = ($isEn && !empty($item->hoc_ham_hoc_vi_en)) ? $item->hoc_ham_hoc_vi_en : $item->hoc_ham_hoc_vi;
                        $displayMajor = ($isEn && !empty($item->chuyen_nganh_en)) ? $item->chuyen_nganh_en : $item->chuyen_nganh;
                        @endphp
                        <tr>
                            <td class="pl-4">
                            ...
                            <td>
                                ...
                                @php
                                $deptName = ($isEn && !empty($entry['department_name_en'])) ? $entry['department_name_en'] : ($entry['department_name'] ?? '');
                                $roleTitle = ($isEn && !empty($entry['chuc_vu_en'])) ? $entry['chuc_vu_en'] : ($entry['chuc_vu'] ?? '');
                                @endphp
                                <span class="badge {{ !empty($entry['is_primary']) ? 'badge-primary' : 'badge-secondary' }} mr-1 mb-1">
                                    {{ $deptName }} @if($roleTitle) ({{ $roleTitle }}) @endif
                                    @if(!empty($entry['is_primary']))<small> ★</small>@endif
                                </span>
                                @endforeach
                                @else
                            ...
                            <td class="text-center pr-4">
                                <a href="{{ route('admin-nhan-su-edit', [app()->getLocale(), $item->_id]) }}" class="btn btn-sm btn-outline-primary mr-1">✏️ {{ __('Sửa') }}</a>
                                <a href="{{ route('admin-nhan-su-delete', [app()->getLocale(), $item->_id]) }}" onclick="return confirm('{{ __('Bạn chắc chắn muốn xóa nhân sự này?') }}')" class="btn btn-sm btn-outline-danger">🗑️ {{ __('Xóa') }}</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7" class="text-center py-4 text-muted">{{ __('Không tìm thấy cán bộ giảng viên phù hợp.') }}</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
