        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
            ...
            <div class="lg:col-span-5 space-y-6">
                <div class="bg-white p-6 sm:p-7 rounded-2xl shadow-sm border border-gray-200">
                    <h2
                        class="text-base sm:text-lg font-bold font-heading text-gray-800 uppercase tracking-wide border-b pb-3 mb-4 border-gray-100 flex items-center gap-2">
                    ...
                    <div class="space-y-4 text-xs sm:text-sm text-gray-600">
                        <div>
                            <strong
                                class="text-gray-900 block font-heading mb-1">{{ __('Khoa Sư phạm - Trường Đại học An Giang') }}</strong>
                        ...
                        <div class="pt-3 border-t border-gray-100 space-y-2.5">
                            <p class="flex items-center gap-3">
                                ...
                                <span><strong>Email:</strong> <a href="mailto:peda@agu.edu.vn"
                                        class="text-agu-blue hover:underline">peda@agu.edu.vn</a></span>
                            </p>
                            ...
                            <p class="flex items-center gap-3">
                                ...
                                <span><strong>{{ __('Điện thoại') }}:</strong> <a href="tel:+842966256565"
                                        class="text-agu-blue hover:underline">+84 296 6256565 (ext 1917)</a></span>
                            </p>
                            ...
                            <p class="flex items-center gap-3">
                                ...
                                <span><strong>{{ __('Giờ làm việc:') }}</strong>
                                    {{ __('Thứ Hai - Thứ Sáu (07:00 - 17:00)') }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-2 sm:p-3 rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <iframe
                        ...
                        width="100%" height="260" style="border:0; border-radius: 0.75rem;" allowfullscreen=""
            ...
            <div class="lg:col-span-7">
                <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-200">
                    <div class="mb-6">
                        <span
                            class="inline-block px-3 py-1 rounded-full bg-blue-50 text-agu-blue text-xs font-semibold uppercase tracking-wider mb-2">
                    ...
                    <form id="contactFeedbackForm" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="sender_type" required
                                    class="w-full bg-gray-50 border border-gray-300 text-gray-800 rounded-lg px-3.5 py-2.5 text-xs focus:ring-2 focus:ring-agu-blue focus:bg-white outline-none">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="block text-xs font-semibold text-gray-700 mb-1.5">{{ __('Họ và tên') }}:</label>
                                <input type="text" name="fullname"
                                    placeholder="{{ __('Tùy chọn (Để trống nếu muốn ẩn danh)') }}"
                                    class="w-full bg-gray-50 border border-gray-300 text-gray-800 rounded-lg px-3.5 py-2.5 text-xs focus:ring-2 focus:ring-agu-blue focus:bg-white outline-none">
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-semibold text-gray-700 mb-1.5">{{ __('Email hoặc Số điện thoại') }}:</label>
                                <input type="text" name="contact"
                                    placeholder="{{ __('Tùy chọn (Để nhận phản hồi trực tiếp)') }}"
                                    class="w-full bg-gray-50 border border-gray-300 text-gray-800 rounded-lg px-3.5 py-2.5 text-xs focus:ring-2 focus:ring-agu-blue focus:bg-white outline-none">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="content" rows="4" required
                                placeholder="{{ __('Mô tả chi tiết ý kiến, phản ánh hoặc sáng kiến của bạn...') }}"
                                class="w-full bg-gray-50 border border-gray-300 text-gray-800 rounded-lg px-3.5 py-2.5 text-xs focus:ring-2 focus:ring-agu-blue focus:bg-white outline-none resize-none"></textarea>
                        </div>

                        <div class="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                            <div id="contactMsg" class="text-xs font-semibold"></div>
                            <button type="submit" id="btnSendContact"
                                class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-agu-blue hover:bg-blue-700 text-white font-bold px-7 py-3 rounded-lg text-xs shadow-md transition-all active:scale-95">
                    ...
                    @foreach ($publishedFeedbacks as $item)
                        <div class="bg-gray-50 rounded-xl p-5 border border-gray-200/70 space-y-3">
                            <div class="flex flex-wrap items-center justify-between gap-2 text-xs">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="px-2.5 py-0.5 rounded-full bg-blue-100 text-agu-blue font-medium">
                                        @if ($item->topic == 'dao-tao')
                                            {{ __('Đào tạo') }}
                                        @elseif($item->topic == 'co-so-vat-chat')
                                            {{ __('Cơ sở vật chất') }}
                                        @elseif($item->topic == 'viec-lam')
                                            {{ __('Tuyển dụng & Việc làm') }}
                                        @else
                                            {{ __('Khác') }}
                                        @endif
                                    </span>
                            ...
                            @if ($item->response_content)
                                <div
                                    class="bg-white border-l-4 border-agu-blue p-4 rounded-r-lg text-xs sm:text-sm shadow-sm space-y-1.5">
                                    <div class="flex items-center justify-between text-xs text-agu-blue font-bold">
                                        <span>{{ __('Phản hồi từ:') }}
                                            {{ $item->responder_name ?: ($item->assigned_to ?: __('Ban Chủ nhiệm Khoa')) }}</span>
                ...
                @if (method_exists($publishedFeedbacks, 'hasPages') && $publishedFeedbacks->hasPages())
                    <div class="mt-8 pt-4 border-t border-gray-100 flex items-center justify-center">
                        <nav class="flex items-center gap-1.5" aria-label="Pagination">
                            ...
                            @else
                                <a href="{{ $publishedFeedbacks->previousPageUrl() }}"
                                    class="w-9 h-9 flex items-center justify-center rounded-lg bg-white border border-gray-200 text-gray-700 hover:bg-agu-blue hover:text-white hover:border-agu-blue transition shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 19l-7-7 7-7" />
                                    </svg>
                            ...
                            @foreach ($publishedFeedbacks->getUrlRange(1, $publishedFeedbacks->lastPage()) as $page => $url)
                                @if ($page == $publishedFeedbacks->currentPage())
                                    <span
                                        class="w-9 h-9 flex items-center justify-center rounded-lg bg-agu-blue text-white font-bold text-xs shadow-sm">
                                        {{ $page }}
                                    </span>
                                @else
                                    <a href="{{ $url }}"
                                        class="w-9 h-9 flex items-center justify-center rounded-lg bg-white border border-gray-200 text-gray-700 hover:bg-blue-50 hover:text-agu-blue text-xs font-semibold transition shadow-sm">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                            ...
                            @if ($publishedFeedbacks->hasMorePages())
                                <a href="{{ $publishedFeedbacks->nextPageUrl() }}"
                                    class="w-9 h-9 flex items-center justify-center rounded-lg bg-white border border-gray-200 text-gray-700 hover:bg-agu-blue hover:text-white hover:border-agu-blue transition shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 5l7 7-7 7" />
