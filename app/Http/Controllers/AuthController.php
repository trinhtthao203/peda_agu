...
class AuthController extends Controller
{
  ...
  function authenticate(Request $request, $locale = ''){
    ...
    if (Auth::attempt(['username' => $data['username'], 'password' => $data['password'], 'active' => 1])) {
      ...
      // Normalize _id key: Laravel MongoDB may return 'id' or '_id' as string/ObjectId
      $userId = null;
      if (isset($user[0]['_id'])) {
        $userId = is_object($user[0]['_id']) ? (string) $user[0]['_id'] : $user[0]['_id'];
        $user[0]['_id'] = $userId;
      } elseif (isset($user[0]['id'])) {
        $userId = is_object($user[0]['id']) ? (string) $user[0]['id'] : $user[0]['id'];
        $user[0]['_id'] = $userId;
      }
      $request->session()->put('user', $user[0]);
      $logQuery = array (
        'action' => 'Đăng nhập hệ thống',
        'id_collection' => $userId,
        'collection' => 'users',
        'data' => $user[0]
      );
  ...
  function mobile_login(Request $request){
    ...
    if (Auth::attempt(['email' => $data['email'], 'password' => $data['password'], 'active' => 1])) {
      ...
      if (!isset($user[0]['_id']) && isset($user[0]['id'])) {
        $user[0]['_id'] = is_object($user[0]['id']) ? (string) $user[0]['id'] : $user[0]['id'];
      } elseif (isset($user[0]['_id']) && is_object($user[0]['_id'])) {
        $user[0]['_id'] = (string) $user[0]['_id'];
      }
      $request->session()->put('user', $user[0]);
      $logQuery = array (
        'action' => 'Đăng nhập hệ thống',
        'id_collection' => $user[0]['_id'] ?? null,
        'collection' => 'users',
        'data' => $user[0]
      );
      LogController::addLog($logQuery);
  ...
  function logout(Request $request, $locale = ''){
    $id_user = $request->session()->get('user._id') ?? $request->session()->get('user.id');
