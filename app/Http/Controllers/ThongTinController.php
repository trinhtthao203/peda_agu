...
class ThongTinController extends Controller
{
    ...
    public static function get_sdg_tags()
    {
        ...
        foreach (self::$sdg_tags as $key => $value) {
            $translatedTags[$key] = __($value);
        }
        return $translatedRoles = $translatedTags;
    }
    ...
    function delete(Request $request, $locale = '', $id = '')
    {
        ...
        if (!$data) {
            Session::flash('msg', 'Không tìm thấy bài viết');
            return redirect(env('APP_URL') . $locale . '/admin/thong-tin');
        }
        ...
        if (!empty($data['attachments'])) {
            foreach ($data['attachments'] as $file) {
                if (isset($file['aliasname']) && $file['aliasname']) {
                    \Illuminate\Support\Facades\Storage::delete('public/files/' . $file['aliasname']);
                    $absolutePath = storage_path('app/public/files/' . $file['aliasname']);
